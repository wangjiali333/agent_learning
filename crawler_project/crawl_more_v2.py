...
if __name__ == '__main__':
    ...
    # 爬取更多数据给较少的分类
    tasks = [
        ('糖果巧克力', '2083810791223459840', ['奶糖', '薄荷糖', '口香糖']),
        ('服装', '2088620594038833152', ['女装', '男装', '羽绒服']),
        ('饮料', '2083190855883030528', ['可乐', '雪碧', '可乐']),
    ]

    for cat_name, tno, keywords in tasks:
        print(f'\n{"="*50}')
        print(f'爬取: {cat_name}')
        print(f'{"="*50}')

        current = db.connection.cursor()
        current.execute('SELECT COUNT(*) FROM productioninfo WHERE tno = %s', (tno,))
        ...
        for kw in keywords:
            print(f'\n  搜索: {kw}')
            added = crawl_by_keyword(kw, tno, max_pages=3)
            print(f'  新增: {added} 条')
            time.sleep(1)
