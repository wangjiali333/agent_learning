...
class SearchTool(Tool):
    ...
    1. 混合模式 (hybrid) - 智能选择TAVILY或SERPAPI或ARxiv搜索
    ...
    def __init__(
        ...
        super().__init__(
            name="search",
            description="一个智能网页搜索引擎。支持混合搜索模式，自动选择最佳搜索源。当前支持的搜索源有：TAVILY、SERPAPI、ARxiv。当你需要回答关于时事、事实,科研论文以及在你的知识库中找不到的信息时，应使用此工具。",
        )
        self.backend = backend
        self.tavily_key = tavily_key or os.getenv("TAVILY_API_KEY")
        self.serpapi_key = serpapi_key or os.getenv("SERPAPI_API_KEY")
        self.available_backends = []  # 可用的搜索方式
    ...
    def _setup_backends(self):
        ...
        # 加入Arxiv搜索方式
        self.available_backends.append("arxiv")
        ...
        elif self.backend not in ["tavily", "serpapi", "arxiv", "hybrid"]:
    ...
    def run(self, parameters: Dict[str, Any]) -> str:
        ...
        try:
            ...
            elif self.backend == "arxiv":
                if "arxiv" not in self.available_backends:
                    return self._get_api_config_message()
...
def search(query: str, backend: str = "hybrid") -> str:
    ...
    Args:
        query: 搜索查询关键词
        backend: 搜索后端 ("hybrid", "tavily", "serpapi", "arxiv")
...
def search_arxiv(query: str) -> str:
    """使用Arxiv进行论文搜索"""
    return search(query, backend="arxiv")
